...
class Raytracer(object):
    ...
    def rtRayColor(self, intercept, rayDirection, recursion=0):
        ...
        if material.matType == OPAQUE:
            for light in self.lights:
                if light.lightType == "Ambient":
                    ...
                    if light.lightType == "Directional":
                        ...
                        lightDir = ml.twoVecSubstraction(light.point, intercept.point)
                        lightDir = ml.vecNorm(lightDir)
            ...
            outside = ml.twoVecDot(rayDirection, intercept.normal) < 0
            
            #Agregar un margen de error
            bias = ml.vecMultEsc(intercept.normal, 0.001)
            
            #Generar rayos de reflexion
            reflect = ml.reflectVector(intercept.normal, [i*-1 for i in rayDirection])
            reflectOrigin =  ml.twoVecSum(intercept.point, bias) if outside else ml.twoVecSubstraction(intercept.point, bias)
            ...
            if not totalInternalReflection(intercept.normal, rayDirection, 1.0, material.ior):
                ...
                refractOrigin =  ml.twoVecSubstraction(intercept.point, bias) if outside else ml.twoVecSum(intercept.point, bias)
                ...
                reflectColor = ml.vecMultEsc(reflectColor, kr)
                refractColor = ml.vecMultEsc(refractColor, kt)
